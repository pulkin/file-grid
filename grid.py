...
parser.add_argument("-t", "--static-files", nargs = "+", help = "files to be replicated in each copy of the job", metavar = "FILENAME")
...
parser.add_argument("action", help = "action to perform", choices = ["new", "run", "cleanup", "which", "optimize", "progress", "distribute"])
...
if options.action == "new" or options.action == "optimize" or options.action == "distribute":
    ...
    if options.action == "distribute":
        ...
        if os.path.exists(filename_data) and not options.c:
            print("The grid is already present in this folder. Use 'grid cleanup' to cleanup previous run or -c to continue previous run.")
    ...
    Float = Combine( Optional(Literal('+') | Literal('-')) + Numeric + Optional( "." + Optional(Numeric) ) + Optional( CaselessLiteral('E') + Integer ) ).setParseAction(
    ...
    GroupHeader = Optional(Combine(VariableName("id") + ZeroOrMore( Suppress(":") + VariableName)("flags")) + Suppress("="))
            ...
            for i in self.__statements__:
                ...
                if isinstance(i, EscapeStatement):
                    ...
                    try:
                        ...
                        logging.warn("Expression named '{name}' at {st} could not be evaluated and will be ignored".format(
                            st = i,
                            name = i.id,
                        ))
    ...
    logging.info("Total: {fixed} fixed statement(s) ({comb} combination(s)), {opt} optimize statement(s) and {dep} dependent statement(s)".format(
    ...
    for x in glob.glob(options.prefix+"*"):
        if not x in grid_state["grid"]:
            print("File or folder {name} may conflict with the grid. Either remove it or use a different prefix through '--prefix' option.".format(name = x))
    ...
    if options.action == "optimize":
        ...
        if len(statements_fix)>0:
            warn("Found {n} fixed groups which will be ignored during optimize run. Use 'grid new' option if you want to unpack fixed groups.".format(n = len(statements_fix)))
        ...
        if not "target_error" in grid_state["optimize"]:
            ...
            if options.target:
                ...
                for i in range(len(vector)):
                    ...
                    if v0 == v1:
                        print("Problem with a finite difference gradient: the values from '{g0}' and '{g1}' are equal to {val}".format(
                            g0 = f0,
                            g1 = f1,
                            val = v0,
                        ))
                        logging.error("Finite difference gradient vanishes. Value obtained in '{g0}' and '{g1}' is {val}".format(
                            g0 = f0,
                            g1 = f1,
                            val = v0,
                        ))
...
# ----------------------------------------------------------------------
    ...
    if options.files or options.static_files or options.prefix or options.target:
        parser.error("-f, --files, -t, --static-files, -p, --prefix, -g, --target options are meaningless for {action} action".format(action = options.action))
    ...
    for f in current_state["grid"]:
        ...
        try:
            ...
            logging.exception("Failed to execute {command} (working directory {directory})".format(command = command,directory = cwd))
            sys.exit(1)
            
        logging.info("  executed {command} in {directory} with PID:{pid}".format(command = command, directory = cwd, pid = p.pid))
...
# ----------------------------------------------------------------------
    ...
    for p1, p2 in zip(grid[:-1], grid[1:]):
        ...
        for parameter in data_x.keys():
            total += (p1[2][parameter] - p2[2][parameter])**2 / (data_x_range[parameter][1] - data_x_range[parameter][0])**2
